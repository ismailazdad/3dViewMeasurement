...
class MeasurementDistance extends Measurement {
	constructor( container) {
		console.log('MeasurementDistance constructor')
		super(container);
		// this.container = container;

		this.createGizmo = function(container) {
			console.log('MeasurementDistance createGizmo')
			console.log('container',container)
			this.measurementGizmo = new MeasurementGizmoDistance(this,  container );
			// console.log('this.measurementGizmo ',this.measurementGizmo )
			return this.measurementGizmo;
		};

		this.getValue = function() {
			console.log('MeasurementDistance getValue')
			// console.log('this.measurementGizmo.getValue()',this.measurementGizmo.getValue())
			return this.measurementGizmo.getValue();
		};

		this.getInfo = function() {
			console.log('MeasurementDistance getInfo')
		...
		this.getType = function() {
			console.log('MeasurementDistance getType')
			return 'Distance';
		};

		this.getDescription = function() {
			console.log('MeasurementDistance getDescription')
			var value = this.getValue();
			if (value == null) return "distance";
			console.log('description','distance = ' + this.getValue().toFixed(2))
...
class MeasurementGizmoDistance extends MeasurementGizmo {
	constructor(measurement, container) {
		console.log('MeasurementGizmoDistance constructor')
		super();
		...
		this.handleGizmos = {
			...
			'STARTLINE': [
				new Mesh( new CylinderGeometry( 0.5, 0.5, 1, 4, 1, false ), new MeasurementGizmoMaterial( { color: 0x000000, opacity: 0.1 } ) ),
				new Vector3( 0, 0.5, 0 ),
				new Vector3( Math.PI/2, 0, 0 )
			],
			'ENDLINE': [
			...
			'STARTARROW': [
				new Mesh( new CylinderGeometry( 3, 0, 6, 8, 1, false ), new MeasurementGizmoMaterial( { color: 0xff0000, opacity: 0.4 } ) ),
				new Vector3( 0, 1, 0 ),
				new Vector3( Math.PI/2, 0, 0 )
			],
			'ENDARROW': [
		...
		this.acceptPoints = function() {
			// console.log('MeasurementGizmoDistance acceptPoints')
			return this.controlPoints.length < 3;
		}

		this.mustDragGizmo = function() {
			console.log('MeasurementGizmoDistance mustDragGizmo')
		...
		this.mustSnapToPart = function() {
			console.log('MeasurementGizmoDistance mustSnapToPart')
			return this.controlPoints.length < 2;
		}

		this.update = function ( camera ) {
			console.log('MeasurementGizmoDistance update')
			//update gizmos
			if (this.isVisible()) {
				this.show();
		...
		this.onGizmoMoved = function(gizmo, offset) {
			console.log('MeasurementGizmoDistance onGizmoMoved')
			switch (gizmo) {
				case "TOPLINE":
				case "TEXT":
	...
	addControlPoint(point, object, forceAdd, face, callbackAddedObject) {
		console.log('MeasurementGizmoDistance addControlPoint')
		...
		if (this.controlPoints.length == 1) {
			console.log('MeasurementGizmoDistance case 1')
			...
			console.log('MeasurementGizmoDistance case 2')
			//get control points in world coordinates
	...
	// this.updateGizmosFromControlPoints = function(camera) {
		updateGizmosFromControlPoints(camera) {
		console.log('MeasurementGizmoDistance updateGizmosFromControlPoints')
		var scope = this;
		var distance;
		//getting width
		...
		console.log('MeasurementGizmoDistance scope.controlPoints.length',scope.controlPoints.length)
		switch (scope.controlPoints.length) {
			case 4:
				console.log('MeasurementGizmoDistance scope.controlPoints.length  4')
			...
			//
			case 2:
				console.log('MeasurementGizmoDistance scope.controlPoints.length  2')
				//end point
			...
			case 1:
				console.log('MeasurementGizmoDistance scope.controlPoints.length  1')
				//start point
